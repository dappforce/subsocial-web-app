...
>>>>>>> ecd648a... AddTeamMemberFormik, BlogsBySlug (not ready)
...
=======
    ...
    const renderDropDownMenu = () => {
        ...
        const menu = (
            <Menu>
                {isMyBlog && <Menu.Item key='0'>
                    <Link href={`/blogs/[id]/edit`} as={`/blogs/${id.toString()}/edit`}><a
                        className='item'>Edit</a></Link>
                </Menu.Item>}
                {edit_history.length > 0 && <Menu.Item key='1'>
                    <div onClick={() => setOpen(true)}>View edit history</div>
                </Menu.Item>}
    ...
    const NameAsLink = () => <Link href={`/blogs/[${(slug ? 'slug' : 'blogId')}]`}
                                   as={`/blogs/${(slug ? slug : id)}`}><a>{name}</a></Link>;
...
>>>>>>> ecd648a... AddTeamMemberFormik, BlogsBySlug (not ready)
        ...
        return <>
            <div className={`DfBlogStats ${isMyBlog && 'MyBlog'}`}>
                ...
                <div onClick={() => setFollowersOpen(true)}
                     className={'DfStatItem DfGreyLink ' + (!followers && 'disable')}>
    ...
    const NewPostButton = () => isMyBlog ?
        <Button href={`/blogs/${id}/posts/new`} icon='plus' size='small' className='DfGreyButton'>New
            post</Button> : null;
...
=======
        ...
        {followersOpen && <BlogFollowersModal id={id} accountsCount={blog.followers_count} open={followersOpen}
                                              close={() => setFollowersOpen(false)}
                                              title={<Pluralize count={followers} singularText='Follower'/>}/>}
        {renderPostPreviews()}
    </Section>;
...
ViewBlogPage.getInitialProps = async (props): Promise<any> => {
    ...
    console.log('QWE Slug: ', props.query.slug)
    let idBySlug = await api.query.blogs.blogIdBySlug('merov') as any;
    console.log('QWE ID by slug: ', idBySlug)
