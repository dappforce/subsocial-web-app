  ...
  switch (index) {
    case ElasticIndex.spaces:
      return (
        <ViewSpace
          key={`${id}-${i}`}
          spaceData={unknownData as SpaceData}
          preview
          withFollowButton
        />
      );
    case ElasticIndex.posts: {
      ...
      return (
        <PostPreview
          key={postData.post.struct.id.toString()}
      ...
      return (
        <Segment>
          <ProfilePreviewWithOwner
            key={`${id}-${i}`}
            address={id}
            owner={unknownData as ProfileData}
          />
        </Segment>
      );
    default:
...
const InnerSearchResultList = <T extends DataResults>(
